...
Camera *Parser::parseLensCamera()
{
  Point eye( 0.0, 0.0, 0.0 );
  Point lookat( 0.0, 1.0, 0.0 );
  Vector up( 0.0, 0.0, 1.0 );
  double hfov = 90.0;
  double focal_length = 15.0;
  double aperture = 2.8;
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "eye" ) )
        ...
        hfov = parseReal();
      else if ( peek( "focal_length" ) )
        focal_length = parseReal();
  ...
  return new LensCamera( eye, lookat, up, hfov, focal_length, aperture );
}
...
Material *Parser::parsePhongMaterial()
{
  ...
  return new PhongMaterial( color, Kd, Ka, Ks );
}
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "material" ) )
        ...
        throwParseException( "Expected `material', `p0', `p1', `p2' or }." );
    }
  return new Polygon( material, p0, p1, p2 );
}
  ...
  string &filename )
{
  ...
  for ( ; ; ) {
    if ( peek( "filename" ) )
      ...
      scene->setSS( parseInteger());
    else if ( peek( "dof" ) )
      scene->setDOF( parseInteger());
    else if ( peek( "start" ) )
      scene->setStart( parseReal());
    else if ( peek( "end" ) )
      scene->setEnd( parseReal());
    else if ( peek( "xres" ) )
      xres = parseInteger();
    else if ( peek( "yres" ) )
      yres = parseInteger();
